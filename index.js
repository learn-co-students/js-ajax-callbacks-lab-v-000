  ...
  showCommits = function(link) {
    ...
    .done(function(data) {
      for (var i = 0; i < data.length; i++) {
        ...
        var author = data[i]["commit"]["author"]["name"] || "None provided";   
        var authorLogin = data[i]["author"]["login"] || "None provided";    
        var avatar = data[i]["author"]["avatar_url"] || "None provided";
        $("#details").append(`<div class="commits"><p><b>Commit number ${num}</b></p><p>SHA: ${sha}</p><p></p><p>Author: ${author}</p><p>Author Login: ${authorLogin}<p><img src=${avatar} style="width: 50%; height: 50%"/></p><hr></div>`); 
        }
      }).fail(function(error) {
        displayError();
    })
